...
class Decompose:
    ...
    def kernel_pca(self, data, n_components, kernel='linear',
                   degree=3):  # kernel = ‘linear’, ‘poly’, ‘rbf’, ‘sigmoid’, ‘cosine’, ‘precomputed’
        scaler = MinMaxScaler()
        scaler.fit(data)

        scaled_data = scaler.transform(data)
        obj = KernelPCA(n_components=n_components, kernel=kernel, degree=degree)
        obj.fit(scaled_data)
        x = obj.transform(scaled_data)
        return x
